...
@jaseci_action(act_group=["flow"], allow_remote=True)
def select_response(state_ext_item: dict, state_response: list, dial_context: dict , prev_response: Union[str, None]):
    ...
    if state_ext_item:
        ...
        for item in state_ext_item:
            if item not in context_key:
                ...
                # print("state_ext_item[item]")
                # print(state_ext_item[item])
                if prev_response in state_ext_item[item] and len(state_ext_item[item]) > 1: state_ext_item[item].remove(prev_response)
                ...
                break
    if response == "":
        if prev_response in state_response and len(state_response) > 1: state_response.remove(prev_response)
        # print(state_response.remove(prev_response))
        response = random.choice(state_response)
        dic["name"]= response_name
        dic["response"]= response

        # if dic["response"] !="" and dic["response"] != prev_response and len(response) > 1:
        #     break
        # else:
            
        #     if (i > len(state_response)):
        #         # print("break")
        #         break
        #     i += 1
...
# sei = {}
# sr = ['Would you like to add anything else to your memory?', "Is there anything else on your mind that you'd like to add?", 'Anything else?']
# dc = {}
# pr= "Anything else?"

# dd = select_response(sei, sr, dc, pr)
# print(dd)
...
# Keep selecting a random item until it is different from the previously selected item
    if prev_response in response_list and len(response_list) > 1: response_list.remove(prev_response)
    # i = 0
    # while True:
